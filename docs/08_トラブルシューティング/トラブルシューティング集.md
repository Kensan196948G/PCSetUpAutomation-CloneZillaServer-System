...
# 4. 必要なディレクトリを作成
sudo mkdir -p /tftpboot/nbi_img/pxelinux.cfg
sudo chmod -R 755 /tftpboot

# 5. DRBL設定を再実行
sudo /usr/sbin/drblsrv -i
sudo /usr/sbin/drblpush -i
# 対話モードでは enp2s0 のみを選択（docker0は選択しない）
...
**詳細ガイド**:
- [DRBL_FIX_DOCKER_GUIDE.md](../04_インフラ/DRBL_FIX_DOCKER_GUIDE.md)
- 自動修正スクリプト: `scripts/fix_drbl_docker_issue.sh`

**予防策**:
- DRBL環境ではDockerサービスを無効化する
- drblpush実行時に物理NICのみを選択する
- tftpd-hpaを使用し、atftpdは削除する
...
| 日付 | バージョン | 更新内容 |
|------|-----------|---------|
| 2025-11-19 | 1.1 | Docker干渉問題（PXE-000）を追加 |
| 2025-11-17 | 1.0 | 初版作成 |
