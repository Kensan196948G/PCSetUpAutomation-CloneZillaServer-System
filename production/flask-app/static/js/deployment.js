...
function startDeployment(image, mode, selectedPCs) {
    ...
    btn.disabled = true;
    btn.innerHTML = '<span class="spinner-border spinner-border-sm me-2"></span>展開作成中...';

    // Step 1: Create deployment configuration
    const createData = {
        name: `展開_${image}_${new Date().toISOString().slice(0, 19).replace('T', '_')}`,
        image_name: image,
        mode: mode,
        target_serials: selectedPCs.map(pc => pc.serial),
        created_by: 'Web UI'
    };
    ...
    .then(data => {
        ...
        return fetch(`/api/deployment/${deploymentId}/start`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            }
    ...
    .catch(error => {
        ...
        alert('展開の開始に失敗しました: ' + error.message);
        btn.disabled = false;
        btn.innerHTML = '<i class="bi bi-play-circle-fill me-2"></i>展開を開始';
    });
}
