...
def image_management():
    ...
    try:
        ...
        # Get images from DRBL
        images_raw = drbl_client.list_images()
        ...
        for img in images_raw:
            images.append({
                'name': img['name'],
                'path': img['path'],
                'description': f"{img['disk_count']}個のディスク, {len(img.get('partitions', []))}パーティション",
                'size': img['size_human'],
                'created_date': img['created'],
                'is_active': False  # TODO: Implement default image tracking
            })

        # Set first image as active if any
        if images:
            images[0]['is_active'] = True

        return render_template('deployment/images.html', images=images)

    except Exception as e:
        logger.error(f'Error loading images: {e}')
        flash('Failed to load images', 'error')
        # Return empty list on error
        return render_template('deployment/images.html', images=[])
